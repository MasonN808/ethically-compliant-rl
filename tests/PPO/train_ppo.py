...
class Cfg(TrainCfg):
    ...
    seed: int = 7
    ent_coef: float = 0
    env_logger_path: str = "tests/PPO/env_logger.txt"
    # env_logger_path: str = None
    rng_logger_path: str = "tests/PPO/rng_logger.txt"

    # Logging advantages
    with open("weights_ppo.txt", "w") as file:
        pass
...
def train(args: Cfg):
    ...
    # with open(args.rng_logger_path, 'w') as file:
    #     # file name
    ...
    agent = PPO("MlpPolicy",
                 ...
                 seed=args.seed,
                 rng_logger_path=args.rng_logger_path)
    ...
    for i in range(args.epochs):
        ...
        if i % args.model_save_interval == 0:
            path = f"tests/PPO/models/{args.wandb_project_name}/{run.id}/model_epoch({i})"
    ...
    # Write the state to a text file
    # with open(args.rng_logger_path, 'a') as file:
    #     # file name
    #     file.write(f"File: {__file__}\n")
    #     # current line number
    #     file.write(f"Line: {inspect.currentframe().f_lineno}\n")
